...
\descript{金牌讲师 | AI 开发 | 应用开发 | IoT嵌入式 }
...
\begin{tightemize}
    \item 设计\textbf{四智能体协作管线}（场景感知→严重度评估→RAG 责任判定→报告生成），集成 MCP 协议工具调用与交通法规知识库，实现事故端到端自动分析定责
...
\runsubsection{OpenWork — AI 智能工作助手}
\descript{架构设计 \& 全栈开发}
\location{2026.03 - 2026.06 | 个人项目}
\begin{tightemize}
    \item 自研\textbf{AI Agent 框架}，实现多轮工具调用循环，基于 XML 标签解析替代 JSON Function Calling，支持 OpenAI 兼容协议与流式响应
    \item 基于 Vue 3 + Monaco Editor + Electron 构建\textbf{四包单体仓库}架构，设计三合一文件系统抽象层，运行时自动适配 IPC/REST/Browser FSA，支持 Web 与桌面双模式部署
    \item 集成\textbf{MCP 协议}（stdio/SSE/HTTP），实现外部工具自动发现与注册；设计滑动窗口会话记忆系统，支持 Markdown/PDF/Office 等多格式文档处理与 AI 辅助工作流，支持Skills、RAG
\end{tightemize}
\sectionsep
